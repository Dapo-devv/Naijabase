...
export default function MarketPage() {
  ...
  // 🚀 CHANGE: Start with an empty object instead of loading today's data
  ...
  // 🚀 CHANGE: Only fill inputs IF the user clicked "Edit".
  // On page load (editingDate is null), this forces the inputs to stay blank.
  useEffect(() => {
    ...
    if (editingDate) {
      ...
      // Always reset to blank when not editing (including page reloads!)
      const cleared = {};
      data.marketItems.forEach((it) => {
        cleared[it] = "";
      });
    ...
    <div className="max-w-4xl mx-auto px-4 py-6 animate-fade-in space-y-8">
      <div>
        <h1 className="text-2xl font-extrabold text-neutral-text flex items-center gap-2">
          <ShoppingCart className="w-6 h-6 text-primary" /> Market Price Logger
        </h1>
        <p className="text-sm text-gray-500">
          Track prices of everyday items over time.
        </p>
      </div>
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-5">
        {isEditing && (
          <div className="mb-4 flex items-center gap-2 text-sm text-secondary-600 bg-secondary-50 px-3 py-2 rounded-lg">
            <Pencil className="w-4 h-4" /> Editing log for{" "}
        ...
        >
          ...
          {isEditing
            ? `Update Prices for ${formatDate(editingDate)}`
            : hasTargetLog
        ...
        {saved && (
          <p className="text-center text-sm text-green-600 mt-2 animate-fade-in">
            {isEditing
              ? "Prices updated for selected date."
              : "Prices logged for today."}
          </p>
        )}
      </div>
      ...
      <div>
        <h2 className="text-lg font-bold text-neutral-text flex items-center gap-2 mb-4">
          <Calendar className="w-5 h-5 text-primary" /> Past Market Logs
        </h2>
        {sortedLogs.length === 0 ? (
          <div className="bg-gray-50 rounded-xl p-8 text-center text-gray-400 border border-dashed">
            <p className="text-sm">
              No logs yet. Log your first market prices above!
            </p>
          </div>
        ) : (
              ...
              return (
                ...
                >
                  <div className="flex-1">
                    ...
                    <p className="text-sm text-gray-500">
                      {Object.keys(log.prices).length} items · Total spent:{" "}
                      {naira(dailyTotal)}
                    </p>
                  </div>
